<#
.SYNOPSIS
    GUI installer for the pd2-mods bundle.
.DESCRIPTION
    A small WinForms window that auto-detects your PAYDAY 2 folder, lets you
    confirm/change it, then downloads the mods from GitHub and installs them.
    Compile to an .exe with build.ps1 (uses PS2EXE).
#>
...
$install.Location = New-Object System.Drawing.Point(345, 345)
$install.Size = New-Object System.Drawing.Size(95, 30)
...
$install.Add_Click({
    ...
    try {
        ...
        Add-Log "Downloading mods from GitHub ($Repo)..."
        ...
        Add-Log 'Installing SuperBLT loader (WSOCK32.dll)...'
        $dll = Join-Path $src 'WSOCK32.dll'
        if (Test-Path $dll) { Copy-Item $dll -Destination $gameDir -Force }
        $progress.Value = 80

        Add-Log 'Installing mods folder...'
        $dstMods = Join-Path $gameDir 'mods'
        if (-not (Test-Path $dstMods)) { New-Item -ItemType Directory -Path $dstMods -Force | Out-Null }
        Copy-Item -Path (Join-Path $src 'mods\*') -Destination $dstMods -Recurse -Force
        $progress.Value = 95
        ...
        [System.Windows.Forms.MessageBox]::Show('Mods installed successfully!', 'PAYDAY 2 Mods Installer',
    ...
    finally { $install.Enabled = $true; $browse.Enabled = $true }
})
